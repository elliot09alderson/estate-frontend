...
import { Share2, MapPin, Home, DollarSign, Maximize, Bed, Bath, Eye, ExternalLink, Search } from 'lucide-react';
...
const Portfolio = () => {
  ...
  const handleShare = async () => {
    const portfolioUrl = `${window.location.origin}/agent/${user?._id}`;
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {data?.data?.total || 0} {(data?.data?.total || 0) === 1 ? 'property' : 'properties'} in your portfolio
              </p>
            </div>
            <Button
              onClick={handleShare}
            ...
            <div className="flex flex-wrap gap-6 items-center">
              ...
              <div className="ml-auto">
                <p className="text-xs text-muted-foreground">
                  {filteredProperties.length} of {allProperties.length} {allProperties.length === 1 ? 'property' : 'properties'}
                </p>
              </div>
            </div>
          </motion.div>
            ...
            <div className="max-w-md mx-auto">
              ...
              {allProperties.length === 0 ? (
                <>
                  ...
                  <p className="text-sm text-muted-foreground mb-6">Start building your portfolio by adding your first property.</p>
                ...
                <>
                  ...
                  <p className="text-sm text-muted-foreground mb-6">Try adjusting your search terms or filters.</p>
                  <Button
                    onClick={() => {
                      setSearchQuery('');
                    ...
                    <Card className="overflow-hidden hover:shadow-lg transition-all duration-300 group border-border/50 h-full">
                      <div className="relative h-56 overflow-hidden bg-muted">
                        ...
                        <div className="absolute top-3 left-3 flex gap-2">
                          ...
                          {!property.isActive && (
                            <Badge variant="destructive" className="text-xs font-medium px-2 py-0.5">Inactive</Badge>
                          )}
                          {property.isApproved === 'pending' && (
                            <Badge className="bg-amber-500/90 text-white backdrop-blur-sm border-0 text-xs font-medium px-2 py-0.5">Pending</Badge>
                          )}
                          {property.isApproved === 'rejected' && (
                            <Badge variant="destructive" className="text-xs font-medium px-2 py-0.5">Rejected</Badge>
                          )}
                        </div>
